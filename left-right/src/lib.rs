...
pub trait Absorb<O> {
    ...
    /// added to the internal oplog, but applied to the first map directly using `absorb_second`.
    /// The first `publish` then calls `sync_with` instead of `absorb_second`.
    ///
    /// This method should then ensure that `self`'s state exactly matches that of `first`. Be
    /// particularly mindful of non-deterministic implementations of traits that are often assumed
    /// to be deterministic (like `Eq` and `Hash`), and of "hidden states" that subtly affect
    /// results like the `RandomState` of a `HashMap` which can change iteration order.
    fn sync_with(&mut self, first: &Self);
}
